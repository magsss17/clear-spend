...
struct ApprovedMerchant: Identifiable {
    ...
    let totalSpentToday: Int
    let lastUpdate: Int
    
    init(name: String, category: String, icon: String, isVerified: Bool, reputationScore: Double, businessLicenseVerified: Bool, communityRating: Double, trustLevel: TrustLevel, verificationDate: Date, dailyLimit: Double, monthlyTransactions: Int, fraudReports: Int, isApproved: Bool = true, parentApproved: Bool = true, totalSpentToday: Int = 0, lastUpdate: Int = Int(Date().timeIntervalSince1970)) {
        self.id = UUID()
        self.name = name
        ...
        self.parentApproved = parentApproved
    ...
    // Computed property for icon based on category
    ...
    // Computed property for daily usage percentage
    var dailyUsagePercent: Double {
        guard dailyLimit > 0 else { return 0 }
        return min(Double(totalSpentToday) / Double(dailyLimit) * 100, 100)
    }
    ...
    static let examples = [
        ...
        ApprovedMerchant(
            ...
            communityRating: 8.5,
            trustLevel: .good,
            verificationDate: Date().addingTimeInterval(-15*24*60*60),
            dailyLimit: 80.0,
            monthlyTransactions: 18,
            fraudReports: 0,
            isApproved: true,
            parentApproved: true,
            totalSpentToday: 15000000,
            lastUpdate: Int(Date().timeIntervalSince1970)
        ),
        ApprovedMerchant(
            name: "Steam",
